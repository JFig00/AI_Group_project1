...
class Checker(TwoPlayerGame):
    ...
    def make_move(self, pos):
        ...
        new_white_pieces = []
        new_black_pieces = []
        # convert board array into player data
        for x,y in black_squares:
            if self.board[x,y] == 'W':
                new_white_pieces.append((x,y))
            elif self.board[x,y] == 'B':
                new_black_pieces.append((x,y))

        self.players[0].pos = new_white_pieces
        self.players[1].pos = new_black_pieces
    ...
    def lose(self):
        ...
        if self.current_player == 1:
            for x,y in self.white_territory:
                if self.board[x,y] == 'B':
                    return True
            return False
        else:
            for x,y in self.black_territory:
                if self.board[x,y] == 'W':
                    return True
            return False
